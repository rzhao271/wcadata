# Valid best average times for each event
# Horizontal box plot
...
# Get count of average times from all current events
# Leave out FMC and MBLD
event_times = dict()
valid_events = {'222', '333', '444', '555', '666', '777', 
                '333oh', '333bf', '444bf', '555bf',
                'clock', 'minx', 'pyram', 'skewb', 'sq1'}
cur.execute('SELECT eventId, best FROM RanksAverage')
...
def plot(columns, tick_step, file_name):
    ...
    ax.set_ylabel('Competitor best average times in seconds')
    ax.set_xlabel('Event')
    ax.set_title('WCA average solve times per event')
    plt.savefig(f"{img_dir}/{file_name}", bbox_inches='tight')
    fig.clear()

plot(columns, 120, 'quartiles-averages.png')

# Now filter out 4BLD and 5BLD
zoom1_columns = [column for column in columns if column[0] not in ['444bf', '555bf']]
plot(zoom1_columns, 30, 'quartiles-averages-zoom1.png')
...
plot(zoom2_columns, 5, 'quartiles-averages-zoom2.png')
